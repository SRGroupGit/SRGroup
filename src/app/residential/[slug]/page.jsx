...
import { fetchResidential } from '@/lib/features/residential';
...
export default function ResidentialSolo() {
  ...
  const detail = useSelector((state) => state.residential);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        await dispatch(fetchResidential(params.slug));
      } catch (error) {
            ...
            <div className='m-auto  mt-4 flex h-fit max-w-screen-2xl flex-col gap-4 md:flex-row'>
              ...
              <div className=' h-fit w-full overflow-hidden rounded-xl bg-blue-200 p-8  text-white  md:w-1/3'>
                ...
                ${emailError ? 'text-red-500' : ' opacity-0 '} text-[10px]`}
                      ...
                      <div className=' detail.datas-center gap-7 md:flex'>
                        <div className=' w-full'>
                          <label
                            className={`
